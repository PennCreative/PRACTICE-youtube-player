...
// Cards
// We are creating a template here for WHEN we loop through the array. THIS DOES NOT LOOP THROUGH
const cardsOnDom = (array) => {
  let domString = ""; //We are looping through AND clearing the domstring! ALWAYS (for this exercise)
...
const eventListeners = () => {
  ...
  document.querySelector("#cardContainer").addEventListener("click", (e) => {
    ...
    if (e.target.id) {
      ...
      //Splitting the ID and the vidId in the Data
      // Go to code snacks for more destruction
      // console.log(e.target.id.split('--'));
      //BELOW: is the GOLD STANDARD
      const [method, videoId] = e.target.id.split("--");

      // find the index of the object in the array: findIndex
      ...
      //Splice is the best method for Deletion because is takes out and reorders the array
      if (e.target.id.includes("delete")) {
        console.log("Delete Button Pressed");
        data.splice(index, 1);
        cardsOnDom(data); //Google Confirm
